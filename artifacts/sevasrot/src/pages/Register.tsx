...
export default function Register() {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    registerMutation({ data: values }, {
      onSuccess: (res) => {
        login(res.token, res.user);
        toast({ title: "Welcome to Sevasrot!", description: "Account created successfully." });
        setLocation("/");
      },
      onError: (err: any) => {
        toast({ 
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center">
            ...
            <h2 className="text-3xl font-display font-bold text-foreground">Join Sevasrot</h2>
            <p className="mt-2 text-muted-foreground">Create an account to support our mission</p>
          </div>
